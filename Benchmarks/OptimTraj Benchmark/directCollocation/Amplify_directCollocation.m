function soln = Amplify_directCollocation(soln)
% modified directCollocation to get problem size

problem = soln.problem;
...
%%%% Store the results:
assert(numel(zLow) == numel(zUpp));
[c, ceq] = P.nonlcon(zGuess); % --------------- Amplify 6/15/26
soln.info.amplify_ndv = numel(zGuess); % --------------- Amplify 6/15/26
soln.info.amplify_nc = numel(c); % --------------- Amplify 6/15/26
soln.info.amplify_nceq = numel(ceq); % --------------- Amplify 6/15/26
soln.info.amplify_nzbnd = numel(zUpp); % --------------- Amplify 6/15/26
end
